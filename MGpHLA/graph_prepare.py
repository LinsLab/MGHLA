...
#os.environ['TORCH_HOME']='E:\Pretrained'
...
# data prepare
def protein_graph_construct_24(proteins, save_dir):
    # Load ESM-1b model
    # torch.set_grad_enabled(False)
    model, alphabet = pretrained.esm1b_t33_650M_UR50S()
...
if __name__ == '__main__':
    ...
    # # for test
    dataset = 'contact'
    if dataset in ['contact', 'davis']:
        #proteins = json.load(open('data/' + dataset + '/common_hla_key_full_sequence_new.txt'), object_pairs_hook=OrderedDict)
        proteins = json.load(open('../data/contact/common_hla_key_full_sequence_new.txt'), object_pairs_hook=OrderedDict)
    ...
    save_dir='../data/pre_process/contact/distance_map'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    protein_graph_construct_24(proteins, save_dir)
