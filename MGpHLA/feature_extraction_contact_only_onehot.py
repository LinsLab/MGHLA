...
def seq_feature_only_onehot(seq):      #序列特征
    pro_hot = np.zeros((len(seq), len(pro_res_table)))
    for i in range(len(seq)):
        # if 'X' in pro_seq:
        #     print(pro_seq)
...
def sequence_to_graph(target_key, target_sequence, distance_dir):
    ...
    target_feature = seq_feature_only_onehot(target_sequence[24:])
...
def batch_seq_feature_only_onehot(seq_list,max_len,emb_len):
    # Assume all input tables and one_hot_encoding_unk function are defined outside this function
    batch_features = []
    ...
    for seq in seq_list:
        ...
        if padding_length!=0:
            for _ in range(padding_length):
                padding_feature.append([0] *len(pro_res_table))
                
            padding_feature=np.array(padding_feature)
            seq_feature=np.concatenate((pro_hot,padding_feature),axis=0)
