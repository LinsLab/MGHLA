...
struc_dropout = 0.1
struc_dropout2=0.1
d_model=32  #peptide d_model
class MGHLA(torch.nn.Module):
...
class classtopo_GNN(torch.nn.Module):
    def __init__(self,num_features_hla=32,hidden_channels=64,dropout=0.1):
        ...
        self.fc=nn.Linear(num_features_hla,112).to(device)
...
def important_nodes_new_nodeedge(hla_feature, hla_batch):
    residue_indices_0 = torch.tensor([6, 8, 23, 44, 58, 61, 62, 65, 66, 68, 69, 72, 73, 75, 76, 79, 80, 83, 94, 96, 98, 113, 115, 117, 142, 146, 149,
                                      151, 155, 157, 158, 162, 166, 170], device=hla_feature.device)
    residue_indices_1 = torch.tensor([6, 8, 23, 44, 58, 61, 62, 65, 66, 68, 69, 72, 73, 75, 76, 79, 80, 83, 94, 96, 98, 113, 115, 117, 142, 146, 149,
                                      151, 155], device=hla_feature.device)

    # 获取每个 batch 的大小
    unique_list, counts = torch.unique(hla_batch, return_counts=True)
    
    # 根据大小分割 hla_feature
    hla_split_feature = torch.split(hla_feature, tuple(counts))
    ...
    return new_hla_feature, hla_batch